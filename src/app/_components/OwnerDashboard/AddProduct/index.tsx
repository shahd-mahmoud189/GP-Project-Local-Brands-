...
const onImageUpload = useCallback(
  ...
  const totalStock = variantsEnabled
    ? fields.reduce(
        (sum, _, idx) => sum + (watch(`Variants.${idx}.stockQuantity`) || 0),
        0
      )
    : watch("StockQuantity") || 0;
  ...
  return (
    <form
      ...
      className="container mx-auto px-6 py-4 space-y-6"
    >
      {/* Basic Info & Image */}
      <div className="bg-white rounded-2xl border border-[#E8E4E0] p-6 shadow-sm space-y-6">
        <h3 className="font-bold text-lg text-[#2D2D2D] flex items-center gap-2">
...
function useImageScan() {
  ...
  const handleFiles = useCallback((files: File[]): void => {
    ...
    setStatus("scanning");
    setTimeout(() => {
      const hasBlocked = files.some((f) =>
        f.name.toLowerCase().includes("ai")
      );
      setStatus(hasBlocked ? "blocked" : "authentic");
    }, 1500);
  }, []);

  return { status, inputRef, handleFiles };
}