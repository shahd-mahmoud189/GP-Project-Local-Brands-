...
//                                 <tr
//                                     key={product.id}
//                                     className={`hover:bg-gray-50 transition-colors ${isLowStock && product.status === "Approved"
//                                             ? "bg-amber-50/50"
//                                             : ""
//                                         }`}
//                                 >
                                ...
                                >
                                    <td className="px-5 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="max-w-[200px]">
                                                <p className="text-sm font-bold text-[#2D2D2D] truncate">{product.productName}</p>
                                                <p className="text-[10px] text-gray-400 font-medium uppercase tracking-tight">#{product.productId}</p>
                                                <p className="text-[11px] text-gray-500 line-clamp-1 mt-0.5">{product.description}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-4 py-4 hidden lg:table-cell">
                                    ...
                                    <td className="px-4 py-4">
                                        <span className="text-sm font-bold text-[#BC5439]">EGP {product.basePrice.toLocaleString()}</span>
                                    </td>
                                    <td className="px-4 py-4">
                                        <div className="flex flex-col gap-1">
                                            <span className={`text-sm font-bold ${totalStock === 0 ? "text-red-600" : totalStock < 5 ? "text-amber-600" : "text-[#2D2D2D]"}`}>
                                                {totalStock} units
                                            </span>
                                            {isLowStock && product.approvalStatusText === "Approved" && (
                                                <span className="flex items-center gap-1 text-[10px] text-amber-600 font-bold uppercase">
                                                    <AlertTriangle className="w-3 h-3 shrink-0" />
                                                    Low Stock
                                                </span>
                                    ...
                                    <td className="px-4 py-4">
                                        <StatusBadge status={product.approvalStatusText as ProductStatus} rejectReason={product.rejectionReason} />
                                    </td>
                                    <td className="px-5 py-4 text-right">
                                        <div className="flex items-center justify-end gap-1">
                                            <button type="button" onClick={() => onEdit(product)} className="w-8 h-8 rounded-full flex items-center justify-center text-gray-400 hover:bg-[#E8E4E0] hover:text-[#2D2D2D] transition-all">
                                                <Pencil className="w-3.5 h-3.5" />
                                            </button>
                                            <button type="button" onClick={() => onDelete(product.productId)} className="w-8 h-8 rounded-full flex items-center justify-center text-gray-400 hover:bg-red-50 hover:text-red-600 transition-all">
                                                <Trash2 className="w-3.5 h-3.5" />
                                            </button>
                                        </div>
                                    </td>
